    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        <div className="bg-[#0f172a] p-4 border border-[#1e293b] shadow-xs">
          <span className="text-[11px] font-bold uppercase tracking-wider text-[#64748b]">Gross Collections</span>
        ...
        <div className="bg-[#0f172a] p-4 border border-[#1e293b] shadow-xs">
          ...
          <div className="text-2xl font-black text-[#f8fafc] font-mono mt-1">{transactions.length}</div>
          <p className="text-[10px] text-[#94a3b8] mt-0.5 uppercase">All Settlement Attempts</p>
        </div>
      ...
      <div className="bg-[#0f172a] border-2 border-[#1e293b] shadow-xs p-5 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-2 bg-[#0f172a] border border-[#1e293b] text-xs text-[#f8fafc] placeholder-[#A89885] font-mono focus:outline-none focus:border-[#8b5cf6]"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="px-3 py-2 bg-[#0f172a] border border-[#1e293b] text-xs text-[#382B20] focus:outline-none focus:border-[#8b5cf6]"
            >
            ...
            <button
              ...
              className="px-3 py-2 bg-[#0f172a] border border-[#1e293b] hover:bg-[#FAF6EE] text-xs font-bold text-[#382B20] flex items-center gap-1.5 transition-colors cursor-pointer"
            >
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#0f172a]">
              {loading ? (
                ...
                filtered.map((tx) => (
                  <tr key={tx.id} className="hover:bg-[#020617] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="font-bold text-[#f8fafc]">{tx.payerName}</div>
                      <div className="text-[11px] font-mono text-[#94a3b8]">{tx.payerEmail}</div>
                    </td>
      ...
      {previewSlip && (
        <div className="fixed inset-0 z-60 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[#0f172a] border-2 border-[#1e293b] p-6 max-w-lg w-full space-y-4">
            <div className="flex justify-between items-center border-b border-[#0f172a] pb-2">
              <h4 className="text-xs font-bold uppercase text-[#f8fafc]">
                Proof of Payment Slip: {previewSlip.filename}
              </h4>
              <button
                type="button"
                onClick={() => setPreviewSlip(null)}
                className="text-xs text-[#64748b] font-bold hover:text-black cursor-pointer"
              >
                ✕
              </button>
            </div>
            <div className="max-h-96 overflow-auto border border-[#1e293b] bg-[#020617] p-2 flex justify-center">
              <img
