...
export class ResendEmailNotificationService implements IEmailNotificationService {
  ...
  public async sendApplicationSubmitted(params: EmailParams): Promise<boolean> {
    ...
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 2px solid #1e293b; background-color: #ffffff;">
        <div style="height: 4px; background: linear-gradient(to right, #8b5cf6, #a855f7); margin: -24px -24px 20px -24px;"></div>
        <h2 style="color: #6366f1; margin-top: 0; text-transform: uppercase; letter-spacing: 0.5px;">Enrollment Application Received</h2>
        ...
        <div style="background-color: #FAF6EE; border-left: 4px solid #8b5cf6; padding: 16px; margin: 20px 0;">
          <p style="margin: 0; font-size: 12px; font-weight: bold; text-transform: uppercase; color: #855D1E;">Your Official Reference Number:</p>
          <p style="margin: 4px 0 0 0; font-size: 22px; font-weight: 900; color: #6366f1; font-family: monospace; letter-spacing: 1px;">${params.referenceNumber}</p>
        </div>
        ...
        <p style="text-align: center; margin: 28px 0;">
          <a href="${params.magicLinkUrl}" style="background-color: #382B20; color: #ffffff; padding: 12px 24px; text-decoration: none; font-weight: bold; text-transform: uppercase; font-size: 12px; letter-spacing: 1px; display: inline-block; border: 1px solid #8b5cf6;">Check Application Status</a>
        </p>
        <hr style="border: none; border-top: 1px solid #0f172a; margin: 24px 0;" />
        <p style="font-size: 11px; color: #94a3b8;">Please keep this email for your records. If you did not initiate this application, please contact admissions@stfrancis.edu immediately.</p>
      </div>
    `;
  ...
  public async sendRevisionRequested(params: EmailParams & { revisionNotes: string }): Promise<boolean> {
    ...
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 2px solid #1e293b; background-color: #ffffff;">
        <div style="height: 4px; background: #8b5cf6; margin: -24px -24px 20px -24px;"></div>
        ...
        <div style="background-color: #FEF9C3; border-left: 4px solid #8b5cf6; padding: 16px; margin: 20px 0;">
        ...
        <p style="text-align: center; margin: 28px 0;">
          <a href="${params.magicLinkUrl}" style="background-color: #8b5cf6; color: #ffffff; padding: 12px 24px; text-decoration: none; font-weight: bold; text-transform: uppercase; font-size: 12px; letter-spacing: 1px; display: inline-block;">Update Application Documents</a>
        </p>
      </div>
    `;
