...
export const EmailPreviewModal: React.FC<EmailPreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-[#0f172a] border-2 border-[#1e293b] shadow-2xl max-w-4xl w-full h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          {/* List */}
          <div className="w-full md:w-80 border-r border-[#1e293b] overflow-y-auto bg-[#020617]">
            {loading ? (
              <div className="p-8 text-center text-[#64748b] text-xs uppercase font-bold">Querying dispatch ledger...</div>
            ) : emails.length === 0 ? (
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 cursor-pointer text-xs space-y-1 transition-colors ${
                        isSelected ? 'bg-[#0f172a] border-l-4 border-[#8b5cf6]' : 'hover:bg-[#0f172a]'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-mono font-bold text-[#f8fafc] truncate max-w-[170px]">
                          {e.recipientEmail}
                        </span>
                      ...
                      <p className="text-[10px] font-mono text-[#64748b] flex items-center gap-1">
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#0f172a] flex flex-col">
            {selectedEmail ? (
              <div className="space-y-4 flex-1">
                <div className="p-4 bg-[#020617] border border-[#1e293b] text-xs space-y-1">
                  <div className="grid grid-cols-6 gap-2">
                    <span className="font-bold text-[#94a3b8] uppercase">Recipient:</span>
                    <span className="col-span-5 font-mono font-bold text-[#f8fafc]">{selectedEmail.recipientEmail}</span>
                  </div>
                  <div className="grid grid-cols-6 gap-2">
                    <span className="font-bold text-[#94a3b8] uppercase">Subject:</span>
                    <span className="col-span-5 font-bold text-[#382B20]">{selectedEmail.subject}</span>
                  </div>
                  <div className="grid grid-cols-6 gap-2">
                    <span className="font-bold text-[#94a3b8] uppercase">Dispatched:</span>
                    <span className="col-span-5 font-mono text-[#64748b]">{new Date(selectedEmail.sentAt).toLocaleString()}</span>
                  </div>
                </div>

                <div className="border border-[#1e293b] p-4 bg-[#0f172a] flex-1 overflow-auto">
                  <h4 className="text-[11px] font-bold text-[#94a3b8] uppercase tracking-wider mb-2 border-b border-[#0f172a] pb-1">
                    Rendered Email Body
                  </h4>
                  <div
              ...
              <div className="flex-1 flex items-center justify-center text-[#64748b] text-xs uppercase font-bold">
